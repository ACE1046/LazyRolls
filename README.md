...
06.03.2025 v0.15.4
* Поддержка ESP32. Пока что тестовый вариант для ESP32-C3. Должно работать с ABCD мотором (другие пока не проверялись). Компиляция в Platformio IDE (в Arduino IDE не проверялось).

10.04.2026 v0.15.6
* Для шаговых двигателей добавлена плавная остановка при движении вниз. В настройках можно включить удержание мотора в течение ~5 сек после остановки, для предотвращения разматывания шторы по инерции.
* Для DC моторов можно регулировать скорость через PWM не только на драйверах с отдельным входом PWM, но и через пины управления вращением, если драйвер поддерживает.
* На esp32c3 отображается температура чипа.
* Добавлена поддержка выбора часового пояса в формате POSIX с учётом перевода на летнее время. Добавлен справочник часовых поясов по городам.
* Диапазон уровней солнца над горизонтом в расписании увеличен до [-10, 10] градусов.
* Добавлена функция принудительного поиска нуля для кнопок и RF пульта. К командам для сброса нуля ('zero' по mqtt, /set?zero по http) добавлены 'gotozero' и /set?gotozero для поиска нуля прямо сейчас.
