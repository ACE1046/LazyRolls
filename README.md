...
* Сторожевой пингер. При включении раз в минуту пингует заданный IP (например, wifi-роутер). Два пропущенных пинга записываются в Log. После 5 пропущенных подряд пингов выполняется указанное действие (лог/переподключение/перезагрузка). Интервал пинга и количество пропущенных пингов можно изменить константами PINGER_INTERVAL_S / PINGER_WARN / PINGER_ACTION в прошивке, при необходимости.
